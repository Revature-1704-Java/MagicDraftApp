...
export class LoginService {
  ...
  public processLogin(email: string, password: string): boolean {
    ...
    this.http.post<User>('http://18.217.166.137:8090/login', myForm).subscribe(res => {
      if (res !== null) {
        this.loggedIn = true;
  ...
  public processRegistration(email: string, password: string) {
    ...
    this.http.post<User>('http://18.217.166.137:8090/signup', myForm).subscribe(res => {
      if (res !== null) {
        this.loggedIn = true;
        this.loggedInUser.email = res.email;
        this.loggedInUser.decks = res.decks;
  ...
  public processLogout() {
    this.http.post('http://18.217.166.137:8090/logout', this.loggedInUser).subscribe();
    this.loggedIn = false;
